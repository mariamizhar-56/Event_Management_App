...
public class EventManagementAppSteps {
	...
	public void an_error_is_shown_containing_the_information_of_the_event() {
		await().atMost(5, TimeUnit.SECONDS).untilAsserted(
				() -> assertThat(eventManagementAppWindow.textBox(LBL_ERROR).text()).isNotEmpty().doesNotContainOnlyWhitespaces());
	}
	...
	public void the_Participant_list_contains_the_updated_Participant() {
		assertThat(eventManagementAppWindow.list(LIST_PARTICIPANT).contents()).anySatisfy(
				e -> assertThat(e).contains(PARTICIPANT_NAME_NEW, DBSteps.PARTICIPANT_EMAIL_2));
	}
